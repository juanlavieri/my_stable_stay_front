    ...
    const fetchStableData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3001/api/stables/${id}`,
        );
        setStableData(response.data);
      } catch (error) {
        console.error(
          "Error fetching stable data:",
    ...
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input
          type="number"
          name="pricePerNight"
          value={stableData.pricePerNight}
          onChange={handleChange}
